        ...
        <v-layout row wrap>
          <v-flex v-for="(item,index) in props.items" :key="index" xs12 sm6 md4 lg3>
            <v-card>
              ...
              <v-list dense>
                <v-list-item>
                  <v-list-item-content>{{$t('roleCode')}}</v-list-item-content>
                  <div class="align-end">{{ item.code }}</div>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>{{$t('roleName')}}</v-list-item-content>
                  <div class="align-end">{{ item.roleName }}</div>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>{{$t('remark')}}</v-list-item-content>
...
export default {
  ...
  methods: {
    ...
    clickDelete(id) {
      this.$axios.post(`/sys-role/destroy-role`, { id })
        .then(result => {
          result.id !== 0
    ...
    clickEdit(item) {
      // this.$router.push({ name: 'role-save', params: item.id });
      // this.$router.push({ path: `/role/save/${item.id}` });
      this.$router.push({ name: 'role-save', query: { id: item.id } });
    },
    clickSet(item) {
      this.$router.push({ name: 'role-set', query: { title: item.roleName, id: item.id } });
    },
    clickSearch() {
