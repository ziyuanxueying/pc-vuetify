    ...
    >
        ...
        <v-layout row wrap>
          <v-flex v-for="(item,index) in props.items" :key="index" xs12 sm6 md4 lg3>
            <v-card>
              ...
              <v-list dense>
                <v-list-item>
                  <v-list-item-content>{{$t('business')}}</v-list-item-content>
                  <v-list-item-content class="align-end">{{ item.shopName }}</v-list-item-content>
                </v-list-item>
                <v-list-item>
                ...
                <v-list-item>
                  <v-list-item-content>{{$t('realName')}}</v-list-item-content>
                  <v-list-item-content class="align-end">{{ item.realName }}</v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>{{$t('passWord')}}</v-list-item-content>
                  <v-list-item-content class="align-end">{{ item.passWord }}</v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>{{$t('roleName')}}</v-list-item-content>
                  <v-list-item-content class="align-end">{{ item.roleName }}</v-list-item-content>
                </v-list-item>
              </v-list>
            </v-card>
          </v-flex>
      ...
      <template v-slot:footer>
        <v-layout mt-2 wrap align-center justify-center>
          ...
          <v-btn text fab small color="blue" :disabled="pagination.page === 1" @click="formerPage">
...
export default {
  ...
  methods: {
    ...
    pageInit() {
      this.$axios.get('/sys-user/find-list', {
        params: {
    ...
    clickDelete(id) {
      this.$axios.post(`/sys-user/destroy-user`, { id })
        .then(result => {
          result.id !== 0
    ...
    clickEdit(item) {
      this.$router.push({ name: 'user-save', query: { id: item.id } });
      // this.$router.push({ path: `/user/save/${item.id}` });
    },
    clickSearch() {
